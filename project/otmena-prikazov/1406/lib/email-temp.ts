// lib/email-temp.ts
// ВРЕМЕННОЕ РЕШЕНИЕ БЕЗ ОТПРАВКИ EMAIL
import { prisma } from '@/lib/prisma'
import crypto from 'crypto'

// Сохраняем ссылки для восстановления в памяти (для разработки)
...
// Функция для получения ссылки (для отладки)
export function getResetLink(email: string) {
  return resetLinks.get(email)
}
